...
# ---------- anti-dup helpers ----------
_punct_re = re.compile(r"[^\wÀ-ÖØ-öø-ÿ]+", flags=re.UNICODE)
...
def _has_letters_or_digits(s: str, min_count: int = 3) -> bool:
    return len(re.findall(r"[A-Za-zÀ-ÖØ-öø-ÿ0-9]", s)) >= min_count


def _similar(a: str, b: str) -> bool:
    a = _punct_re.sub(" ", (a or "").lower()).split()
    b = _punct_re.sub(" ", (b or "").lower()).split()
    if not a or not b:
        return False
    if a == b:
        return True
    ...
    inter = len(sa & sb)
    uni = len(sa | sb)
    return bool(uni) and (inter / uni) >= 0.9


def reduce_near_duplicates(segs, window_ms=2000):
    """Supprime/compacte les répétitions quasi identiques qui se suivent."""
    out = []
    for s in segs:
        s["text"] = _clean_text(s["text"])
        if not _has_letters_or_digits(s["text"]):
            continue
        if out:
            last = out[-1]
            if (
                s["speaker"] == last["speaker"]
                and s["start_ms"] - last["end_ms"] <= window_ms
            ):
                if _similar(last["text"], s["text"]):
...
def split_wav_ffmpeg(in_wav, chunk_ms, overlap_ms):
    ...
    if dur <= chunk_ms:  # pas besoin
        ...
        return [(in_wav, 0)]
    logger.info(
        f"Splitting {in_wav} into chunks of {chunk_ms} ms with {overlap_ms} ms overlap"
    )
...
def build_initial_prompt(
    ctx: dict, primary_lang: str, secondary_lang: str, repeat: int = 1
):
    if not ctx:
        return None
    names = []
    ...
    if not bag:
        return None
    bag = list(dict.fromkeys([x.strip() for x in bag if x and isinstance(x, str)]))
    bag = sum([[x] * int(max(1, repeat)) for x in bag], [])
...
def compute_cluster_centroids(wav_path, diar_segments):
    ...
    for s, e, lab in diar_segments:
        s_idx = int(s / 1000 * sr)
        e_idx = int(e / 1000 * sr)
        s_idx = max(0, min(s_idx, len(wav)))
        e_idx = max(0, min(e_idx, len(wav)))
        if e_idx - s_idx < int(0.5 * sr):
            continue
        seg = wav[s_idx:e_idx]
...
def rename_speakers_by_similarity(
    segs, spk_centroids, voiceprints, threshold=0.76, overrides=None
):
    if overrides:
        for s in segs:
...
def vad_segments(wav_path, aggressiveness=2, frame_ms=30, pad_ms=150):
    logger.info(
        f"Running VAD on {wav_path} (aggr={aggressiveness}, frame_ms={frame_ms})"
    )
...
def embed_segments(wav_path, segments_ms):
    ...
    for s, e in segments_ms:
        s_idx = int(s / 1000 * sr)
        e_idx = int(e / 1000 * sr)
        e_idx = min(e_idx, len(wav))
        if e_idx - s_idx < int(0.5 * sr):
            continue
        seg = wav[s_idx:e_idx]
        emb = enc.embed_utterance(seg)
        embs.append(emb)
        spans.append((s, e))
...
def diarize(wav_path, speech_spans, min_k=1, max_k=8):
    wins = []
    [wins.extend(list(sliding_windows(x))) for x in speech_spans]
    if not wins:
        return []
    embs, spans = embed_segments(wav_path, wins)
    if len(embs) == 0:
        return [(s, e, "SPK1") for s, e in speech_spans]
    ...
    for spk, lst in by_spk.items():
        ...
        merged = []
        for s, e in lst:
            if not merged or s - merged[-1][1] > 400:
                merged.append([s, e])
            else:
...
def transcribe_words(
    ...
    """Décodage Whisper durci contre les boucles et le bruit."""
    use_beam = (int(beam_size) if beam_size else 0) > 1
    crt = float(os.getenv("WHISPER_CRT", "1.35"))
    lpt = float(os.getenv("WHISPER_LPT", "-0.6"))
    nst = float(os.getenv("WHISPER_NST", "0.6"))
    logger.info(
        f"Transcribing {wav_path} (lang={language}, beam={beam_size}, best_of={best_of}, cond_prev={condition_on_previous_text})"
    )
    segments, _ = model.transcribe(
        ...
        beam_size=int(beam_size) if use_beam else None,
        best_of=int(best_of) if not use_beam else None,
...
def words_to_segments(assigned, gap_ms=900):
    ...
    if not assigned:
        return segs
    cur_spk = assigned[0]["speaker"]
    cur_start = assigned[0]["start_ms"]
    tokens = [assigned[0]["word"]]
    last_end = assigned[0]["end_ms"]
    for w in assigned[1:]:
    ...
    if txt:
        segs.append(
            {"start_ms": cur_start, "end_ms": last_end, "speaker": cur_spk, "text": txt}
        )
...
def reconcile_speakers(prev_segs, new_segs, seam_start_ms, window_ms=5000):
    if not prev_segs or not new_segs:
        return new_segs
    prev_window = []
    new_window = []
    for s in prev_segs:
...
def process_one_wav(
    ...
    language = (
        None
        if (lang_cfg and lang_cfg.get("multilang"))
        else (lang_cfg.get("primary") if lang_cfg else None)
    )
...
def main(indir, outdir):
    ...
    files = [
        ...
        if p.is_file()
        and p.suffix.lower()
        in [".mp3", ".m4a", ".wav", ".mp4", ".mpeg", ".mpga", ".ogg", ".webm"]
    ]
    ...
    # Trim d'overlap: par défaut on coupe une partie de l'overlap au début des chunks > 0
    default_trim = max(0, chunk_overlap - 800)
    trim_head_ms = int(os.getenv("TRIM_OVERLAP_HEAD_MS", str(default_trim)))
    ...
    for src in tqdm(files, desc="Traitement"):
        ...
        need_split = (dur_ms > max_chunk_min * 60 * 1000) or (
            file_size_mb(str(tmp)) > max_wav_mb
        )
        chunks = (
            split_wav_ffmpeg(str(tmp), max_chunk_min * 60 * 1000, chunk_overlap)
            if need_split
            else [(str(tmp), 0)]
        )
        ...
        for i, (chunk_wav, offset) in enumerate(chunks):
            logger.info(
                f"Processing chunk {i+1}/{len(chunks)}: {chunk_wav} (offset={offset} ms)"
            )
            cond_prev = i == 0  # seul le 1er chunk conditionne sur le texte précédent
            ...
            # applique l'offset global
            for s in segs:
                s["start_ms"] += offset
                s["end_ms"] += offset

            # coupe la tête du chunk courant pour éviter la redite de l'overlap
            if i > 0 and trim_head_ms > 0:
                cutoff = offset + trim_head_ms
                before = len(segs)
                segs = [s for s in segs if s["end_ms"] > cutoff]
                logger.info(
                    f"Trimmed {before - len(segs)} segments at chunk head (cutoff={cutoff} ms)"
                )

            # diar pour centroides ultérieurs
            diar_shifted = [(s + offset, e + offset, lab) for (s, e, lab) in diar]
            diar_all.extend(diar_shifted)

            # réconciliation aux jointures
            if i > 0:
                seam = offset
                segs = reconcile_speakers(
                    all_segs,
                    segs,
                    seam_start_ms=seam,
                    window_ms=min(5000, chunk_overlap + 2000),
                )

            all_segs.extend(segs)

        # compactage anti-répétitions proches
        all_segs = reduce_near_duplicates(all_segs, window_ms=2000)

        # mapping SPKx -> noms (si empreintes vocales)
        overrides = (
            (ctx_global.get("overrides", {}) or {}).get("speakers", {})
            if ctx_global
            else {}
        )
        if voiceprints and diar_all:
            centroids = compute_cluster_centroids(str(tmp), diar_all)
            all_segs = rename_speakers_by_similarity(
                all_segs,
                centroids,
                voiceprints,
        ...
        all_segs = [s for s in all_segs if _has_letters_or_digits(s["text"])]

        # sauvegardes
        out_json = Path(outdir) / f"{base}.json"
        ...
        save_md(
            all_segs, Path(outdir) / f"{base}.md", f"Transcription diarisée — {base}"
        )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--context", required=False, help="Chemin vers context.yaml|json (optionnel)"
    )
    args = ap.parse_args()
    if args.context:
